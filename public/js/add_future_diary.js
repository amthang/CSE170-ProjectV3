...
function initializePage() {
	$('.future a').click(function(e) {
		// Prevent following the link
		e.preventDefault();

		// Get the div ID, e.g., "project3"
		var projectID = $(this).closest('.future').attr('id');
		// get rid of 'project' from the front of the id 'project3'
		var idNumber = projectID.substr('future'.length);
		...
		function addProjectDetails(project_json) {
			...
			var details_div = $('#future' + idNumber + ' .details');
			// add the content to the DIV
			details_div.html(new_html);

			details_div.find('.future-delete').click(function(e) {
